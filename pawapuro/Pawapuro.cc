...
#define GENIUS_DENOMINATOR 2 //天才型出現判定の分母
#define DAIJYOUBU_1 11       //ダイジョーブ博士出現率(rand()%10<DAIJYOUBU_1)
#define DAIJYOUBU_2 11       //ダイジョーブ博士成功率(rand()%10<DAIJYOUBU_2)
...
string Name;                                           //選手名
int Position, sub_days, days, Number, practice_number; //ポジション、変数、経った日にち、背番号、練習項目番号
//能力値↓
int Ball, Control, Stamina, Slider, Curve, Fork, Sinker, Shoot, Trajectory, Meet, Power, Run, Sholder, Protect, Catching;
//練習レベル(投手)↓
int Ball_level = 1, Control_level = 1, Stamina_level = 1, Evolving_level = 1;
//練習レベル(野手)↓
int Meet_level = 1, Power_level = 1, Run_level = 1, Sholder_level = 1, Protect_level = 1, Catching_level = 1;
//練習レベルカウンター(投手)↓
int Ball_level_counter, Control_level_counter, Stamina_level_counter, Evolving_level_counter;
//練習レベルカウンター(野手)↓
int Meet_level_counter, Power_level_counter, Run_level_counter, Sholder_level_counter, Protect_level_counter, Catching_level_counter;
//能力値の評価(G,F,E,D,C,B,A,S)↓
char Control_status, Stamina_status, Trajectory_status, Meet_status, Power_status, Run_status, Sholder_status, Protect_status, Catching_status;
...
int muscle_point, agile_point, technique_point, evolving_point, spirit_point; //筋力、敏捷、技術、変化球、精神それぞれの獲得したポイント変数
...
int condition, condition_point, prolog; //調子、調子による獲得ポイントの増減、プロログカウンター
int choise, genius, Daijyoubu_count;    //リセット選択肢変数、天才型判定変数、ダイジョーブ博士出現回数変数
double genius_point;                    //天才型時必要経験値減係数変数
//ball_personalityの略、運動型、非運動型の内部
double ball_per = 1, control_per = 1, stamina_per = 1, evolving_per = 1, meet_per = 1, power_per = 1, run_per = 1, sholder_per = 1, protect_per = 1, catching_per = 1;
int main()
{
...
GAME_START:
  ...
  if (getchar() == '\n')
    system("clear");
  cout << "[監督]:「ポジションはどこだっけ？」(数字を入力してください。)▼ " << endl;
  cout << "[1]投手、[2]捕手、[3]一塁手、[4]二塁手、[5]三塁手、[6]遊撃手、[7]左翼手、[8]中堅手、[9]右翼手 ▼ ";
  cout << endl;
  cout << "[ポジション]--> ";
  cin >> Position;
  if (getchar() == '\n')
  {
    ...
    if (Position == 9)
    {
      system("clear");
      Position_name = "右翼手";
      cout << "[監督]:「あ、そっかぁ。右翼手かぁ。まぁ頑張ってくれ。」▼ " << endl;
    }
  }
  if (getchar() == '\n')
  {
    system("clear");
    cout << "[[舞台は高校3年甲子園予選大会1ヶ月前。主人公はラストスパートをかけてチームを甲子園に導くことはできるのだろうか。]]▼ ";
  }
  if (getchar() == '\n')
  {
    system("clear");
    Pawapuro_sad();
    cout << "[" << Name << "]:"
         << "「しかし1ヶ月前にもなって監督に名前もポジションも忘れられるなんて、これもうわっかんねぇなぁ。」▼ ";
  }
  if (getchar() == '\n')
  {
    system("clear");
  ...
  PITCHER:
    ...
    START_PITCHER:
      ...
      if (prolog == 0)
      {
        cout << "[監督]:「" << Name << "は今の実力に満足しているかね？」▼ " << endl;
        cout << "[" << Name << "]: [1]もちろんです！　[2]いや、こんなの僕じゃない！ ▼ " << endl;
        cout << "選択 -> ";
        cin >> choise;
        if (choise == 2)
        {
...
void Pitcher_Ball_decide(void)
{
  ...
  cout << "   " << Ball_muscle << " |   " << Ball_technique_span << Ball_technique << " |      |   " << Ball_spirit_span << Ball_spirit << " ";
}
...
void Pitcher_Control_decide(void)
{
  ...
  cout << "      |   " << Control_technique_span << Control_technique << " |      |   " << Control_spirit_span << Control_spirit << " ";
}
...
void Pitcher_Stamina_decide(void)
{
  ...
  cout << "   " << Stamina_muscle_span << Stamina_muscle << " |      |      |    " << Stamina_spirit << " ";
}
...
void Pitcher_Slider_decide(void)
{
  ...
  cout << "      |   " << Slider_technique_span << Slider_technique << " |  " << Slider_evolving_span << Slider_evolving << " |    " << Slider_spirit << " ";
}
...
void Pitcher_Curve_decide(void)
{
  ...
  cout << "      |   " << Curve_technique_span << Curve_technique << " |  " << Curve_evolving_span << Curve_evolving << " |    " << Curve_spirit << " ";
}
...
void Pitcher_Fork_decide(void)
{
  ...
  cout << "      |   " << Fork_technique_span << Fork_technique << " |  " << Fork_evolving_span << Fork_evolving << " |    " << Fork_spirit << " ";
}
...
void Pitcher_Sinker_decide(void)
{
  ...
  cout << "      |   " << Sinker_technique_span << Sinker_technique << " |  " << Sinker_evolving_span << Sinker_evolving << " |    " << Sinker_spirit << " ";
}
...
void Pitcher_Shoot_decide(void)
{
  ...
  cout << "      |   " << Shoot_technique_span << Shoot_technique << " |  " << Shoot_evolving_span << Shoot_evolving << " |    " << Shoot_spirit << " ";
}

void Batter_Trajectory_decide(void)
{
  //int Trajectory_point, Meet_point, Power_point, Run_point, Sholder_point, Protect_point, Catching_point;
  //-----[弾道判定]-------------------------------------------
  int Trajectory_muscle, Trajectory_technique, Trajectory_spirit;
  int Trajectory_muscle_length = 0, Trajectory_technique_length = 0, Trajectory_spirit_length = 0;
  ...
  cout << "   " << Trajectory_muscle_span << Trajectory_muscle << " |      |   " << Trajectory_technique_span << Trajectory_technique << " |   " << Trajectory_spirit_span << Trajectory_spirit << " ";
}
...
void Batter_Meet_decide(void)
{
  ...
  cout << "    " << Meet_muscle << " |      |   " << Meet_technique_span << Meet_technique << " |   " << Meet_spirit_span << Meet_spirit << " ";
}
...
void Batter_Power_decide(void)
{
  ...
  cout << "   " << Power_muscle_span << Power_muscle << " |      |   " << Power_technique_span << Power_technique << " |      ";
}
...
void Batter_Run_decide(void)
{
  ...
  cout << "   " << Run_muscle_span << Run_muscle << " |   " << Run_agile_span << Run_agile << " |      |      ";
}
...
void Batter_Sholder_decide(void)
{
  ...
  cout << "    " << Sholder_muscle << " |    " << Sholder_agile << " |    " << Sholder_technique << " |      ";
}
...
void Batter_Protect_decide(void)
{
  ...
  cout << "      |    " << Protect_agile << " |    " << Protect_technique << " |    " << Protect_spirit << " ";
}
...
void Batter_Catching_decide(void)
{
  ...
  cout << "      |    " << Catching_agile << " |      |   " << Catching_spirit_span << Catching_spirit << " ";
}
...
void Prolog(void)
{
  ...
  if (getchar() == '\n')
  {
    system("clear");
    Pawapuro_normal();
    cout << "[" << Name << "]:「やっと行ってくれた...。監督たまに変になるから困る。」▼ ";
  }
  if (getchar() == '\n')
  {
    system("clear");
...
void Daijyoubu(void)
{
  ...
  if (getchar() == '\n')
  {
    system("clear");
    Daijyoubu_success();
    cout << "[ダイジョーブ博士]:「ソウデース。アナタヲ改造スルタメニヤッテキタノデース。」▼ ";
  }
  if (getchar() == '\n')
  {
    system("clear");
    Pawapuro_sad();
    cout << "[" << Name << "]:「(なんかすごく怪しいけどちょっとやってみたいな。。。)」▼ ";
  }
  if (getchar() == '\n')
  {
    system("clear");
    Daijyoubu_success();
    cout << "[ダイジョーブ博士]:「シカシ、ソノタメニハ大キナ危険ヲ伴イマス。」▼ ";
  }
  if (getchar() == '\n')
  {
    system("clear");
...
DAIJYOUBU:
  ...
  if (i == 1)
  {
    ...
    if (((rand() % 10 + 1) > 0) && ((rand() % 10 + 1) < DAIJYOUBU_2))
    {
      ...
      { //野手の場合
        //MAX値より上にならないための変数
        int d_meet = 0, d_power = 0, d_run = 0, d_sholder = 0, d_protect = 0, d_catching = 0;
        //MAX値と現能力値の差分
        d_meet = 100 - Meet;
        d_power = 100 - Power;
        d_run = 100 - Run;
  ...
  {
    if (getchar() == '\n')
      cout << "[ダイジョーブ博士]:「ソウデスカ。デハゲドー君、彼ヲ元ノ場所ニ戻シテオイテ下サイ。」▼ " << endl;
    if (getchar() == '\n')
      cout << "[ゲドー君]:「ギョギョ。」▼ " << endl;
    if (getchar() == '\n')
...
void Pitcher_Point_allocate(void)
{
  ...
  int length_evolving;
  int length_spirit;
  int muscle_keep = muscle, technique_keep = technique, evolving_keep = evolving, spirit_keep = spirit;
  int Ball_keep = Ball, Control_keep = Control, Stamina_keep = Stamina, Slider_keep = Slider, Curve_keep = Curve, Fork_keep = Fork, Sinker_keep = Sinker, Shoot_keep = Shoot;
  char Control_keep_status, Stamina_keep_status, Slider_keep_status, Curve_keep_status, Fork_keep_status, Sinker_keep_status, Shoot_keep_status;
...
ALLOCATE_PITCHER:
  ...
  cout << " ================================================================" << endl;
  cout << "| --------------------------  ---------------------------------- |" << endl;
  cout << "|| [球速] | [コン] | [スタ] || [ シュート ] " << Shoot_keep << "  | [スライダー] " << Slider_keep << " ||" << endl;
  cout << "||--------------------------||----------------------------------||" << endl;
  cout << "||   " << Ball_keep;
  cout << "  |  「";
  ...
  cout << "」 || [ シンカー ] " << Sinker_keep << "  | [  カーブ  ] " << Curve_keep << " ||" << endl;
  cout << "||--------------------------||----------------------------------||" << endl;
  cout << "||  km/h  |   " << Control_keep << Control_keepspan << "|   " << Stamina_keep << Stamina_keepspan << "|| [ フォーク ] " << Fork_keep << "  |                ||" << endl;
  cout << "| --------------------------  ---------------------------------- |" << endl;
  cout << " ================================================================" << endl;
  cout << "[能力UP後]" << endl;
  cout << " ================================================================" << endl;
  cout << "| --------------------------  ---------------------------------- |" << endl;
  cout << "|| [球速] | [コン] | [スタ] || [ シュート ] " << Shoot << "  | [スライダー] " << Slider << " ||" << endl;
  cout << "||--------------------------||----------------------------------||" << endl;
  cout << "||   " << Ball;
  cout << "  |  「";
  ...
  cout << "」 || [ シンカー ] " << Sinker << "  | [  カーブ  ] " << Curve << " ||" << endl;
  cout << "||--------------------------||----------------------------------||" << endl;
  cout << "||  km/h  |   " << Control << Controlspan << "|   " << Stamina << Staminaspan << "|| [ フォーク ] " << Fork << "  |                ||" << endl;
  cout << "| --------------------------  ---------------------------------- |" << endl;
  cout << " ================================================================" << endl;
  ...
  cout << "| ----------------------------------------------------- |" << endl;
  cout << "||    現在の経験点    | -> |" << musclespan << muscle << " |" << techniquespan << technique << " |" << evolvingspan << evolving << " |" << spiritspan << spirit << " ||" << endl;
  cout << "| ----------------------------------------------------- |" << endl;
  cout << "| ----------------------------------------------------- |" << endl;
  cout << "|| [1]    [ 球速 ]    | ↑  |";
  Pitcher_Ball_decide();
  ...
  if (allocate_number == 1)
  {
    ...
    {
      Ball_point();
      if ((muscle < 0) || (technique < 0) || (spirit < 0) || (Ball == 165))
      {
        if (Ball == Ball_keep)
        {
          if (getchar() == '\n')
        ...
        cout << "球速値は現在" << Ball << "まで上げることができます。▼ " << endl;
        muscle = muscle_keep;
        technique = technique_keep;
        spirit = spirit_keep;
        Ball = Ball_keep;
  ...
  if (allocate_number == 2)
  {
    ...
    while (1)
    {
      ...
      if ((technique < 0) || (spirit < 0) || (Control == 100))
      {
        ...
        cout << "コントロール値は現在" << Control << "まで上げることができます。▼ " << endl;
        technique = technique_keep;
        spirit = spirit_keep;
        Control = Control_keep;
        break;
  ...
  if (allocate_number == 3)
  {
    ...
    while (1)
    {
      ...
      if ((muscle < 0) || (spirit < 0) || (Stamina == 100))
      {
        ...
        cout << "スタミナ値は現在" << Stamina << "まで上げることができます。▼ " << endl;
        muscle = muscle_keep;
        spirit = spirit_keep;
        Stamina = Stamina_keep;
        break;
  ...
  if (allocate_number == 4)
  {
    ...
    {
      Slider_point();
      if ((technique < 0) || (evolving < 0) || (spirit < 0) || (Slider == 7))
      {
        ...
        cout << "スライダー値は現在" << Slider << "まで上げることができます。▼ " << endl;
        technique = technique_keep;
        evolving = evolving_keep;
        spirit = spirit_keep;
        Slider = Slider_keep;
  ...
  if (allocate_number == 5)
  {
    ...
    {
      Curve_point();
      if ((technique < 0) || (evolving < 0) || (spirit < 0) || (Curve == 7))
      {
        if (Curve == Curve_keep)
        {
          if (getchar() == '\n')
        ...
        cout << "カーブ値は現在" << Curve << "まで上げることができます。▼ " << endl;
        technique = technique_keep;
        evolving = evolving_keep;
        spirit = spirit_keep;
        Curve = Curve_keep;
  ...
  if (allocate_number == 6)
  {
    ...
    {
      Fork_point();
      if ((technique < 0) || (evolving < 0) || (spirit < 0) || (Fork == 7))
      {
        if (Fork == Fork_keep)
        {
          if (getchar() == '\n')
        ...
        cout << "フォーク値は現在" << Slider << "まで上げることができます。▼ " << endl;
        technique = technique_keep;
        evolving = evolving_keep;
        spirit = spirit_keep;
        Fork = Fork_keep;
  ...
  if (allocate_number == 7)
  {
    ...
    {
      Sinker_point();
      if ((technique < 0) || (evolving < 0) || (spirit < 0) || (Sinker == 7))
      {
        ...
        cout << "シンカー値は現在" << Sinker << "まで上げることができます。▼ " << endl;
        technique = technique_keep;
        evolving = evolving_keep;
        spirit = spirit_keep;
        Sinker = Sinker_keep;
  ...
  if (allocate_number == 8)
  {
    ...
    {
      Shoot_point();
      if ((technique < 0) || (evolving < 0) || (spirit < 0) || (Shoot == 7))
      {
        if (Shoot == Shoot_keep)
        {
          if (getchar() == '\n')
        ...
        cout << "シュート値は現在" << Shoot << "まで上げることができます。▼ " << endl;
        technique = technique_keep;
        evolving = evolving_keep;
        spirit = spirit_keep;
        Shoot = Shoot_keep;
...
void Batter_Point_allocate(void)
{
  ...
  int length_technique;
  int length_spirit;
  int muscle_keep = muscle, agile_keep = agile, technique_keep = technique, spirit_keep = spirit;
  int Trajectory_keep = Trajectory, Meet_keep = Meet, Power_keep = Power, Run_keep = Run, Sholder_keep = Sholder, Protect_keep = Protect, Catching_keep = Catching;
  char Trajectory_keep_status, Meet_keep_status, Power_keep_status, Run_keep_status, Sholder_keep_status, Protect_keep_status, Catching_keep_status;
...
ALLOCATE_BATTER:
  ...
  cout << " ================================================================" << endl;
  cout << "| -------------------------------------------------------------- |" << endl;
  cout << "|| [弾道] |[ミート]|[パワー]| [走力] | [肩力] |[守備力]| [捕球] ||" << endl;
  cout << "||--------------------------------------------------------------||" << endl;
  cout << "||   ";
  ...
  cout << "」 ||" << endl;
  cout << "||--------------------------------------------------------------||" << endl;
  cout << "||   " << Trajectory_keep_status << "    |   " << Meet_keep << Meet_keepspan << "|   " << Power_keep << Power_keepspan << "|   " << Run_keep << Run_keepspan << "|   " << Sholder_keep << Sholder_keepspan << "|   " << Protect_keep << Protect_keepspan << "|   " << Catching_keep << Catching_keepspan << "||" << endl;
  cout << "| -------------------------------------------------------------- |" << endl;
  cout << " ================================================================" << endl;
  cout << "[能力UP後]" << endl;
  cout << " ================================================================" << endl;
  ...
  cout << "」 ||" << endl;
  cout << "||--------------------------------------------------------------||" << endl;
  cout << "||   " << Trajectory_status << "    |   " << Meet << Meetspan << "|   " << Power << Powerspan << "|   " << Run << Runspan << "|   " << Sholder << Sholderspan << "|   " << Protect << Protectspan << "|   " << Catching << Catchingspan << "||" << endl;
  cout << "| -------------------------------------------------------------- |" << endl;
  cout << " ================================================================" << endl;
  ...
  cout << "| ----------------------------------------------------- |" << endl;
  cout << "||    現在の経験点    | -> |" << musclespan << muscle << " |" << agilespan << agile << " |" << techniquespan << technique << " |" << spiritspan << spirit << " ||" << endl;
  cout << "| ----------------------------------------------------- |" << endl;
  cout << "| ----------------------------------------------------- |" << endl;
  cout << "|| [1]    [ 弾道 ]    | ↑  |";
  ...
  if (allocate_number == 1)
  {
    ...
    {
      Trajectory_point();
      if ((muscle < 0) || (technique < 0) || (spirit < 0) || (Trajectory == 4))
      {
        ...
        cout << "弾道値は現在" << Trajectory << "まで上げることができます。▼ " << endl;
        muscle = muscle_keep;
        technique = technique_keep;
        spirit = spirit_keep;
        Trajectory = Trajectory_keep;
  ...
  if (allocate_number == 2)
  {
    ...
    {
      Meet_point();
      if ((muscle < 0) || (technique < 0) || (spirit < 0) || (Meet == 100))
      {
        if (Meet == Meet_keep)
        {
          if (getchar() == '\n')
        ...
        cout << "ミート値は現在" << Meet << "まで上げることができます。▼ " << endl;
        muscle = muscle_keep;
        technique = technique_keep;
        spirit = spirit_keep;
        Meet = Meet_keep;
  ...
  if (allocate_number == 3)
  {
    ...
    while (1)
    {
      ...
      if ((muscle < 0) || (technique < 0) || (Power == 100))
      {
        ...
        cout << "パワー値は現在" << Power << "まで上げることができます。▼ " << endl;
        muscle = muscle_keep;
        technique = technique_keep;
        Power = Power_keep;
        break;
  ...
  if (allocate_number == 4)
  {
    ...
    while (1)
    {
      ...
      if ((muscle < 0) || (agile < 0) || (Run == 100))
      {
        ...
        cout << "走力値は現在" << Run << "まで上げることができます。▼ " << endl;
        muscle = muscle_keep;
        agile = agile_keep;
        Run = Run_keep;
        break;
  ...
  if (allocate_number == 5)
  {
    ...
    {
      Sholder_point();
      if ((muscle < 0) || (agile < 0) || (technique < 0) || (Sholder == 100))
      {
        ...
        cout << "肩力値は現在" << Sholder << "まで上げることができます。▼ " << endl;
        muscle = muscle_keep;
        agile = agile_keep;
        technique = technique_keep;
        Sholder = Sholder_keep;
  ...
  if (allocate_number == 6)
  {
    ...
    {
      Protect_point();
      if ((agile < 0) || (technique < 0) || (spirit < 0) || (Protect == 100))
      {
        ...
        cout << "守備力値は現在" << Protect << "まで上げることができます。▼ " << endl;
        agile = agile_keep;
        technique = technique_keep;
        spirit = spirit_keep;
        Protect = Protect_keep;
  ...
  if (allocate_number == 7)
  {
    ...
    while (1)
    {
      ...
      if ((agile < 0) || (spirit < 0) || (Catching == 100))
      {
        ...
        cout << "捕球値は現在" << Catching << "まで上げることができます。▼ " << endl;
        agile = agile_keep;
        spirit = spirit_keep;
        Catching = Catching_keep;
        break;
...
void Pitcher_status(void)
{
  ...
  cout << "                   |    球速    |" << Ballspan << Ball << "km/h | ||" << endl;
  cout << "||                ";
...
void Batter_status(void)
{
  ...
  system("clear");
  cout << "[[大会" << days << "日前]]" << endl;
  cout << "[名前] : " << Name << "   [" << Position_name << "][" << Number << "]" << endl;
  cout << " ================================================================" << endl;
  cout << "| -------------------------------------------------------------- |" << endl;
  cout << "|| [弾道] |[ミート]|[パワー]| [走力] | [肩力] |[守備力]| [捕球] ||" << endl;
  cout << "||--------------------------------------------------------------||" << endl;
